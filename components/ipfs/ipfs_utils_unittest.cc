...
void ValidateReplacements(const std::string& scheme,
  ...
  EXPECT_EQ(ipns_url.ReplaceComponents(replacements3),
            scheme + "://host/path");
...
TEST_F(IpfsUtilsUnitTest, ParseIPFSUri) {
  ...
  url = GURL("ipns://bafybeiemxf5abjwjbikoz4mc3a3dla6ual3jsgpdr4cjr3oz3evfyavhwq");
  EXPECT_EQ(url.scheme(), "ipns");
  EXPECT_EQ(url.host(), "bafybeiemxf5abjwjbikoz4mc3a3dla6ual3jsgpdr4cjr3oz3evfyavhwq");
  EXPECT_EQ(url.path(), "/");
...
TEST_F(IpfsUtilsUnitTest, IpfsUriReplacement) {
  ...
  ValidateReplacements("ipns", "QmfM2r8seH2GiRaC4esTjeraXEachRt8ZsSeGaWTPLyMoG");
  ValidateReplacements("ipfs", "QmfM2r8seH2GiRaC4esTjeraXEachRt8ZsSeGaWTPLyMoG");
